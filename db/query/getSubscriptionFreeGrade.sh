...
echo "SELECT @userId := 1 as userId, \
	grade.id as gradeId, \
	grade.grade_english as gradeName, \
	(CASE WHEN (SELECT user.plan_id FROM user WHERE user.id=1) = 1 \
		THEN 'free' \
		ELSE (SELECT \
				...
				(CASE WHEN ssg.started IS NOT NULL AND ssg.user_id=@userId \
					THEN ssg.started \
					ELSE (SELECT user.date_joined FROM user WHERE user.id=@userId) \
				END) as started \
			) \
	END) \
	FROM grade \
	...
	LEFT JOIN user ON user.id=ssg.user_id;" | $msql
