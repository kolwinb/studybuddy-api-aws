...
router.post('/',function(req,res){
    ...
    if (!req.is('application/json')) {
		...
		*/
								...
								var_status='1';
								//insert subscription
								con.query("INSERT INTO ?? (id,subscriber_id,device_id,mobile,operator,type,status,date_time) VALUES(?,?,?,?,?,?,?,?)",["subscriber",'NULL',var_subscriberId,var_deviceId,var_mobile,var_operator,var_deviceType,var_status,var_dateTime],function (err,result) {
									if (err) {
										console.log(err);
                   						data = {
                   							"status":"error",
